...
## 3. `@fudic/transport` — SDD-16 (parte 2), el shell de tres hilos
...
- [ ] **Contrato de mensajes** — `ReqId`, `RenderRequest`, `RenderMessage` (`stream`|`chunk`|`end`),
  `ControlMessage` (`invalidate`|`version`|`purge`). Tipos puros.
- [ ] **Adaptador de transporte** — `canTransferStream()` (probe `structuredClone(s,{transfer:[s]})`,
...
- [ ] **Manifest** — `RouteManifest.match`, `ManifestEntry` (`dynamic`, `chunk`), `loadManifest(url)`
  desde URL absoluta (fuente única). **Criterio §6.12.**
- [ ] **WW** — `RenderChunk` (interfaz, inyectable), `serveRender(port, req, resolveChunk)`,
...
- [ ] **SW** — `Router`, `RouterConfig`, `createRouter(config)` (única rama cache hit/miss →
  `MessageChannel` por `reqId` → `worker.postMessage(req, [port2])` → `receiveRender` → `tee()` a
  `respondWith` + `cache.put` → `port.close()`). **Criterio §6.10.**
- [ ] **Control** — `controlBus(name?)` sobre `BroadcastChannel`: `post`/`on`(→unsubscribe)/`close`.
  **Criterio §6.11.**
- [ ] **Main** — `registerRenderServiceWorker(url)` (gancho mínimo). La hidratación por traversal
