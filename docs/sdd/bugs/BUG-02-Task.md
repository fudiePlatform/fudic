...
- [x] **1. `warm` no descarga documentos.**
...
- [x] **6. Documentar la semántica nueva de `RouteMode`.**
...
- [x] **7. Seleccionar toda ruta renderizable.**
...
- [x] **10. Unificar las ramas de `buildManifest`.**
...
- [x] **13. `warm` calienta chunks, nunca documentos.**
...
- [x] **14. La clave de páginas es la URL de navegación.**
...
- [x] **15. La decisión deja de mirar `mode`.**
...
- [x] **16. El nonce se aplica al servir, no al renderizar.**
...
- [x] **17. `servePage` se reduce a servir lo persistido.**
...
- [x] **18. Confirmar que preview no dependía de `record.html`.**
...
- [x] **20. Ejecutarlo contra el build real.**
...
- [x] Cobertura de ramas de `router.ts` **superior** a la de `main` (§6: el borrado de
      `servePage`/`pageUrlOf` quita ramas, no debe quitar tests).
- [x] Marcar BUG-02 como `Hecho` en [INDEX.md](./INDEX.md) (tabla + registro de progreso).
- [x] Actualizar [SDD-20](../SDD-20-render-sw.md) §4.2 (el manifest ya no lleva `html`),
