...
- [ ] **1. Las dos salidas no están minificadas.**
...
- [ ] **2. Un chunk minificado se enlaza y renderiza.**
...
- [ ] **3. `minify` en `NestedOutputOptions`.**
...
- [ ] **5. La opción se respeta en los dos sentidos.**
...
- [ ] `swbuild.ts` al **100 %**; `link.ts` y `plugin.ts` no bajan de ramas respecto a `main`.
- [ ] Anotar en el registro de progreso el tamaño **antes y después** de `dist/fudic-sw.js` y
      de los siete `sw/c/*.js`. La medición es la mitad del argumento de §2.5 y hay que
      dejarla escrita, no deducirla.
- [ ] Extremo a extremo: `pnpm --filter @fudic/example-basic exec playwright test` en verde
      — el SW minificado tiene que seguir sirviendo y renderizando igual.
- [ ] Marcar BUG-06 como `Hecho` en [INDEX.md](./INDEX.md) (tabla + registro de progreso).
- [ ] Anotar en [BUG-03](./BUG-03-chunks-compartidos-sw.md) §7 una línea: la minificación del
