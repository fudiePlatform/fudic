...
**Estado:** `Hecho` — las ocho tareas implementadas y con tests ·
**Rama:** `worktree-bug-31-emit-incondicional`

> **Dónde aterrizó T6**, que la tarea sitúa en `component-unused.ts` junto a
> `component-declared.ts`: en `contractDiagnostics` ([registry.ts](../../../packages/compiler/src/emit/registry.ts)),
> no en el pase semántico. El motivo es el mismo que el de los `FUD019x`: qué tag declara un
> `<link>` solo lo puede contestar quien resolvió el grafo, y el pase semántico no lee ficheros.
> T8 se cierra con eso mismo — `warningDiag` ya existía en ese canal.
