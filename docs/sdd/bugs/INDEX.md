...
| [BUG-11](./BUG-11-sin-canal-de-update.md) | Un hijo que recibe un valor no tiene canal de actualización | SDD-15 §3.7, §4.7, §7 · props-spec 76 | `core` · `compiler` | `Listo` |
| [BUG-12](./BUG-12-comentario-razor-en-code.md) | Un comentario Razor dentro de `@code` borra todo el bloque | SDD-08 · SDD-11 | `compiler` | `Listo` |
| [BUG-13](./BUG-13-texto-literal-no-sobrevive.md) | El texto literal del autor no llega intacto al output | SDD-05 · SDD-15 §4.4 · gramática 1, 49 | `compiler` · `ssr` | `Listo` |
...
BUG-13  (independiente: `@@` y entidades en texto. Los tres —11, 12, 13— salen de la
        misma sesión y de la misma página de demostración, y no comparten un fichero:
        11 es el emit del factory, 12 el troceado de `@code`, 13 los runs de texto.)

BUG-12  (independiente: es del troceado de `@code` y del batch de Oxc, no del emit.
        Salió de escribir las páginas de demostración de BUG-11 en `examples/basic`,
        y no comparte un solo fichero con él: pueden ir en paralelo.)

BUG-11  (independiente de los diez anteriores: es el primero que no es de la salida
        del build ni de dev, sino del CONTRATO del runtime de cliente. Va antes que
