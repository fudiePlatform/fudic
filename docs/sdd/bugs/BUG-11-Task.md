...
- [ ] **1. El ejemplo del repo falla al typechequear.**
...
- [ ] **2. `$GlobalAttrs` y la firma de `$attrs`.**
...
- [ ] **3. `export type $Slots` en el virtual de cliente.**
...
- [ ] **6. El contrato sigue siendo estricto.**
...
- [ ] `pnpm typecheck` y `pnpm test` en verde en `@fudic/language-core` y en `@fudic/cli`.
- [ ] `language-core` sigue al **100 %** en las cuatro métricas.
- [ ] Marcar BUG-11 como `Hecho` en [INDEX.md](./INDEX.md) (tabla + registro de progreso).
- [ ] Anotar en [SDD-23 §7](../SDD-23-emisor-ts-virtual.md) que `<slot>` ya tiene contrato, con
