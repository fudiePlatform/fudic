...
- [ ] **3. Test de forma del chunk emitido.**
      ...
      interpolada tiene `a`, tiene entrada `u:`, `c` y `u` llaman a `a()`, el cuerpo de `h`
      **no**; el patrón de asignación de `u` deja los dos primeros huecos vacíos y conserva los
      defaults. **Verlo fallar** (§6.3, §6.4).
...
- [ ] **5. `Controller` gana `u`.**
      ...
      diseño.
- [ ] **6. `FudicElement` gana el punto de entrada.**
...
## Fase 2 — El factory emitido (3)

- [ ] **7. `destructuring()` gana la forma de asignación.**
...
- [ ] **8. `a()`: las escrituras de valor salen del cuerpo de fabricación.**
...
- [ ] **9. La entrada `u` del objeto devuelto.**
...
- [ ] **10. Consumir `PropertyBinding` en un host de componente.**
...
- [ ] **11. Distinguir signal de valor constante.**
...
## Fase 4 — Las specs que decían lo contrario (2)
...
- [ ] **13. Derogar la decisión 76 de props-spec.**
...
- [ ] **14. Goldens regenerados y revisados a mano.**
      `packages/compiler/test/emit/__golden__/{app-badge,app-card,app-button}.client.mjs`. La
      única diferencia esperada es la salida de las escrituras de valor a `a()` y la entrada
      `u`. Cualquier otro cambio es un fallo de la Fase 2, no un golden que actualizar (§6.10).
- [ ] **15. Regresiones y cobertura.**
...
- [ ] Anotar en [SDD-15-Task](../SDD-15-Task-fudic-element-y-emit-de-cliente.md) que el contrato
      del controlador cambió antes de las tareas pendientes de event bindings.
