...
- [ ] **3. Instalar y comprobar que el paquete vacío pasa.**
...
- [ ] **5. Resolución de opciones y fin de línea.**
...
- [ ] **13. Recolección y formateo concurrente de hojas.**
...
- [ ] **18. Contexto de impresión y contenido.**
...
- [ ] **21. Control de flujo y rescate de trivia.**
...
- [ ] **22. `@code`, código inline y directivas de layout.**
      ...
      `@RenderSection(n)`).
- [ ] **23. `<style>` y comentarios Razor sueltos.**
...
- [ ] **24. La puerta: `format` y `formatRange`.**
...
- [ ] **26. Criterios 1, 4 y 5 — idempotencia, estabilidad y ficheros rotos.**
...
- [ ] **27. Criterio 2 — round-trip de AST.**
...
- [ ] **28. Criterio 3 — equivalencia de emit. El test duro.**
      ...
      distinto.
- [ ] **29. Criterios 6 y 7 — regiones opacas y comentarios Razor.**
...
- [ ] **30. Criterios 8, 9 y 10 — CSS, adyacencia y atributos.**
...
- [ ] **31. Criterio 12 — formateo de rango.**
...
- [ ] **32. `fudic fmt` (toca `@fudic/cli`, SDD-22).**
...
- [ ] **33. El handler del servidor (toca `@fudic/language-server`, SDD-24).**
      ...
      su **100 %**.
- [ ] **34. Criterio 11 — paridad editor/CLI.**
...
- [ ] **35. El `.vsix` lleva su propio formateador (toca `fudic-vscode`, SDD-25).**
...
- [ ] `pnpm typecheck`, `pnpm test` y `pnpm build` en verde (incluidas las vueltas a
      `@fudic/cli`, `@fudic/language-server` y `fudic-vscode` de la fase 7).
- [ ] Cobertura **100 %** en las cuatro métricas de `@fudic/formatter`, con
      `coverage.include` sobre `src/**`, y los tres paquetes tocados en la suya.
- [ ] `fudic fmt --check` sobre `examples/basic` sale con cero: las páginas reales del ejemplo
      quedan formateadas por el formateador, que es la única prueba que no envejece.
- [ ] Anotar `FUD0480`–`FUD0499` en el catálogo de SDD-12.
- [ ] Marcar SDD-26 como `Hecho` y anotarlo en [INDEX.md](./INDEX.md) (tabla + registro de
      progreso).
