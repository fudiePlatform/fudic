...
| `FUD0110`–`0129` | 08 | `0110` falta `{` · `0111` `@server(…)` / `@client(…)` (66) · `0114` comentario Razor dentro de `@code` (35.a, BUG-13: ahí se comenta en JS). `0112`/`0113` quemados (estrategias retiradas). |
| `FUD0130`–`0149` | 09 | `0130` construcción Razor no permitida en `<style>` · `0131` llaves CSS desbalanceadas. |
| `FUD0150`–`0169` | 10 | `0150` doctype ≠ `html` · `0151` `<html>`/`<head>`/`<body>` faltante o desordenado · `0152` `<link rel="component">` fuera de `<head>` · `0153` `@code` fuera de `<head>` · `0154` más de un `@code` · `0155` orden top-level inválido en componente · `0156` envoltorio host inválido (decisión 75) · `0157` template única ausente en el envoltorio · `0158` `shadowrootmode` inválido · `0159` más de un `<style>` en el head del componente · `0160` atributo `host` escrito en fuente (marcador de output) · `0161` cuerpo de un `<script>` de **código** (decisión 129): fudic no soporta script en línea, y el cuerpo se tiraba sin emitirse y sin diagnosticarse. Sobre el cuerpo y no sobre el tag —`src` es la forma soportada y viaja entera—, con el vacío y el de solo espacios exentos, y con los **datos fuera de la regla**: `application/ld+json` e `importmap` se emiten verbatim (lista cerrada, `type` sin distinguir mayúsculas; un `type` interpolado no se puede leer y por tanto es código). Servido por las **dos** puertas, la fase semántica y `registry.ts`, como `FUD0199`. |
| `FUD0170`–`0189` | 11 | `0170` error de sintaxis JS/TS de Oxc (span mapeado). |
| `FUD0190`–`0209` | 12 | `0190` atributo duplicado · `0191` custom element sin `<link rel="component">` · `0192` `ref` en bucle · `0193` `@server`/`@client` anidado · `0194` más de un `@server`/`@client` · `0195` interpolación no-primitiva (literal array/objeto) · `0196` import por efecto en zona neutra (warning) · `0197` prop requerida que el host no pasa (sobre el tag de apertura) · `0198` `.prop` que el hijo no declara (sobre el nombre) · `0199` `slot="x"` cuyo padre no declara la ranura, o que no tiene padre componente (sobre el valor). Los tres los sirve **quien resuelve el grafo**: el `ComponentRegistry` los contesta con `propsOf`/`slotsOf`, opcionales, y sin ellos la regla calla — que es lo que hace el language server, donde `$required` y `$intoSlot` ya lo comprueban contra el tipo real (BUG-23 §4.4). |
...
| `FUD0590`–`0619` | 34 | `0590` el valor de `control` no es una expresión `@` (decisión 108; cubre el `control="title"` del prototipo, así que el diagnóstico enseña la migración) · `0591` dos elementos del mismo componente enlazan el mismo nodo (decisión 110), salvo que **todos** sean `<input type="radio">` · `0592` `control` sobre un `<input>` cuyo `type` no porta valor de usuario (`submit`, `reset`, `button`, `image`) o no está soportado (`file`); un `type` **dinámico** dejó de estar aquí en BUG-25 — enlaza con `bindByType` (decisión 109) · `0593` `formassociated` fuera del `<template shadowrootmode>` raíz de un componente (decisión 111) · `0594` `control` dentro de un bucle (decisión 114, hermana de la 31) · `0595` `control` sin un `<form control>` por encima (decisión 115, BUG-25; exento el control-componente). `0596`–`0619` reservados. Ninguno lanza: se anota, se omite **ese** enlace y el fichero se sigue emitiendo. |
| `FUD0620`–`0639` | 39 | `0620` el prerender de una ruta falló: la página no se genera **y el build falla** — sin span, es un error del build, como los `CliError` de SDD-22 · `0621` la mitad de cliente de una ruta lee `data` y la ruta no declara `load`, así que ahí siempre habrá `undefined` (`warning`) · `0622` el nombre de chunk de una ruta (`safeName`) colisiona con el tag de un componente del build. `0623`–`0639` reservados. Un acceso **dinámico** a `data` (`data[k]`, `f(data)`) no produce diagnóstico y manda `data` entero: no se puede legislar cómo escribe un desarrollador. |
| `FUD0660`–`0679` | 37 | Delegación de eventos (decisiones 117–121), los ocho sobre el AST y **ninguno** sobre el checker: `0660` un `$nombre` sin ningún descendiente que declare `delegate:nombre` · `0661` un marcador que ningún ancestro lee · `0662` un nombre que no declara **ninguna** cabecera envolvente (el mensaje las nombra todas) · `0663` `delegate:` fuera de un bucle (hermana de la 31 y la 114) · `0664` dos bucles ofreciendo el mismo nombre al mismo handler, **aunque iteren el mismo tipo** · `0665` un evento que no burbujea, con el sustituto en el mensaje cuando la plataforma tiene uno · `0666` un `$nombre` fuera de la lista de argumentos de un event binding · `0667` un marcador con valor —decidido en la clasificación (SDD-07) y **reenviado** por el analizador, para que las ocho salgan del mismo sitio y no solo de `pnpm build`—. `0668`–`0679` reservados. |
| `FUD0680`–`0699` | 38 | `0680` `inject(X)` de una clase que su módulo ni decora con `@Service` ni registra con `provide`, y que ningún componente de la página provee — un salto de import, el que el `.fud` ya escribe; si el módulo no se resuelve o no se puede leer, **no hay diagnóstico**, porque un error no se inventa sobre una ausencia · `0681` una ligadura de la plantilla lee un nombre inyectado en `@server` en un componente que hidrata: `@server` no llega al chunk, así que el primer `set` rompe sobre un nombre que no existe · `0682` `inject` en una zona y el único `provide` del fichero en la contraria, que nunca corren en el mismo proceso (la zona neutra es la respuesta que propone el mensaje) · `0683` `inject(…)` dentro del `@server` de una **ruta**, donde va `ctx.inject(…)`: `load(ctx)` es la única función `async` del sistema y no tiene contenedor ambiente · `0684` dos `provide` del mismo token en un `@code`, donde el segundo reemplaza al primero en silencio. `0685`–`0699` reservados. Ninguno detiene el emit: el módulo se escribe degradado. |
| `FUD0700`–`0719` | 40 | `0700` el `@code` de un layout contiene algo que no es su declaración de props · `0701` una prop de layout recibe un valor reactivo (`signal` / `computed`): un layout no tiene mitad de cliente que pueda repintarlo · `0702` la ruta no resuelve una prop **requerida** del layout, sobre su `<link rel="layout">` — es el que ancla la bombilla de SDD-36 · `0703` dos layouts de la misma cadena declaran la misma prop con tipos incompatibles. `0704`–`0719` reservados. |
| `FUD0820`–`0849` | 29 | Snippets (`@snippet` / `@render`). La **gramática**: `0820` nombre ausente o con guión —un guión sería una resta en la cabecera de un `@render`— · `0821` faltan los paréntesis de la firma o de los argumentos. El **cuerpo**, cuatro cosas que no puede llevar: `0822` `<style>` · `0823` `@code` · `0824` un `@snippet` anidado · `0825` un `<head>`. La **llamada**, que comprueba la misma función en el build y en el editor: `0826` nombre que no está en el ámbito · `0827` namespace que no declara ningún `<link rel="snippet" as>` · `0828` aridad insuficiente · `0829` argumentos de más · `0830` nominal que no corresponde a ningún parámetro · `0831` parámetro recibido dos veces · `0832` posicional tras nominal · `0833` un `@` dentro de la cabecera. Y el **ámbito**: `0834` dos snippets con el mismo nombre en el global, con span secundario en el primero y cura poniendo `as` · `0835` recursión directa o indirecta, con el ciclo entero en el mensaje · `0836` un `<link rel="snippet">` que no resuelve o que no declara ningún `@snippet`. El **tipo** de los argumentos no está aquí: lo da TypeScript sobre la proyección (SDD-23). `0837`–`0849` reservados. |
...
y por tanto nunca podrá recibirla (sobre el nombre de la prop) · `0203` un `computed` que cruza
a una prop que el hijo **escribe** — un derivado no es escribible (SDD-31 §4.3). Los cuatro los
