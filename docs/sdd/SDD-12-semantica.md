...
| `FUD0660`–`0679` | 40 | `0660` el `@code` de un layout contiene algo que no es su declaración de props · `0661` una prop de layout recibe un valor reactivo (`signal` / `computed`): un layout no tiene mitad de cliente que pueda repintarlo · `0662` la ruta no resuelve una prop **requerida** del layout, sobre su `<link rel="layout">` — es el que ancla la bombilla de SDD-36 · `0663` dos layouts de la misma cadena declaran la misma prop con tipos incompatibles. `0664`–`0679` reservados. |
