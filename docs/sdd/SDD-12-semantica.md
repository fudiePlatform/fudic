...
| `FUD0590`–`0619` | 34 | `0590` el valor de `control` no es una expresión `@` (decisión 106; cubre el `control="title"` del prototipo, así que el diagnóstico enseña la migración) · `0591` dos elementos del mismo componente enlazan el mismo nodo (decisión 108), salvo que **todos** sean `<input type="radio">` · `0592` `control` sobre un `<input>` cuyo `type` no porta valor de usuario (`submit`, `reset`, `button`, `image`), no está soportado (`file`) o es **dinámico** y no se puede decidir en compilación · `0593` `formassociated` fuera del `<template shadowrootmode>` raíz de un componente (decisión 109) · `0594` `control` dentro de un bucle (decisión 112, hermana de la 31). `0595`–`0619` reservados. Ninguno lanza: se anota, se omite **ese** enlace y el fichero se sigue emitiendo. |
