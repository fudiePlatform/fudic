// components/app-grandchild.js — nivel 2 de la cadena de composición.
// Compone <app-greatgrandchild />.
...
const cl = class extends HTMLElement {
  ...
  static c($props) {
    ...
    const s = () => {
      $n1 && ($n1.dataset.live = '');
      trace(`  s() app-grandchild (${$v1}) enganchado`, 'casc');
    };

    return {
      c: () => {
        $n0 = $dom.el('div'); $n0.className = 'node lvl2';
        $n1 = $dom.el('div'); $n1.className = 'tagline'; $n1.textContent = `app-grandchild · ${$v1}`;
        $dom.append($n0, $n1);
        m(); s();
      },
...
customElements.define('app-grandchild', cl);
