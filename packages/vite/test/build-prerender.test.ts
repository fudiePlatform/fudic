 ...
 * load` prerenders it — the route is `dynamic:false` in the manifest (the SW ignores it)
...
describe('vite build — static prerender (mode 1)', () => {
  it('marks the route static (dynamic:false) in the manifest', () => {
    ...
    expect(manifest[0]).toMatchObject({ pattern: '/about', dynamic: false });
  });
